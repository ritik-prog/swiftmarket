...
import { deleteSeller } from "../../../api/seller";

function DeleteSeller({ isOpen, closeModal, sellerId }: any) {
  ...
  const handleDeleteAccount = async () => {
    try {
      ...
      const result = await deleteSeller(sellerId, data);
      if (result.status === "success") {
        CreateToast("deleteseller", result.message, "success");
        closeModal();
        navigate("/seller");
      }
    } catch (error) {}
